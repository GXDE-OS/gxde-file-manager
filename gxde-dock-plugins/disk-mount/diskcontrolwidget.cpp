 ...
 *       There is also a FileUtils::getKernelParameters() in gxde-file-manager.
    ...
    if (diskDevice->removable()) {
        ...
        for (const QString& blDevStr : blDevList) {
            ...
            if (blDev->hasFileSystem() && blDev->mountPoints().isEmpty()) {
                ...
                // blumia: if mount&open enabled and gxde-file-manager also got installed, use gxde-file-manager.
                //         using mount scheme with udisks sub-scheme to give user a *device is mounting* feedback.
                if (mountAndOpen && !QStandardPaths::findExecutable(QStringLiteral("gxde-file-manager")).isEmpty()) {
                    QString mountUrlStr = "mount://fromMountPlugin#udisks://" + blDevStr;
                    QProcess::startDetached(QStringLiteral("gxde-file-manager"), {mountUrlStr});
                    return;
                }
