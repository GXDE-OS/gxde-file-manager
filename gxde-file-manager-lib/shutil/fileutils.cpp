...
{
    /*
     *  return true if exec field contains gxde-file-manager/file-manager.sh of gxde-file-manager desktopFile
    */
    DesktopFile d(desktopFile);
    return d.getExec().contains("gxde-file-manager") || d.getExec().contains("file-manager.sh");
}
...
{
    bool ret = false;
    QString oldPath = QString("%1/%2/%3.%4").arg(QDir().homePath(), ".cache/gxde-file-manager",key,"json");
