...
void AppController::actionOpenAsAdmin(const QSharedPointer<DFMUrlBaseEvent> &event)
{
    ...
    qDebug() << args;
    QProcess::startDetached("gxde-file-manager-pkexec", args);
}
...
void AppController::actionRemoveFMBackground(quint64 winId)
{
    ...
    QString darkBackgroundPath = QDir::homePath() + "/.config/GXDE/gxde-file-manager/background-dark-FullWindow.png";
...
void AppController::actionSetFMBackground(quint64 winId)
{
    ...
    QString lightBackgroundPath = QDir::homePath() + "/.config/GXDE/gxde-file-manager/background-light-FullWindow.png";
    QString darkBackgroundPath = QDir::homePath() + "/.config/GXDE/gxde-file-manager/background-dark-FullWindow.png";
    QDir dir(QDir::homePath() + "/.config/GXDE/gxde-file-manager/");
    if (!dir.exists()) {
        dir.mkpath(QDir::homePath() + "/.config/GXDE/gxde-file-manager/");
    }
    if (QFile::exists(lightBackgroundPath)) {
        QFile::remove(lightBackgroundPath);
    }
