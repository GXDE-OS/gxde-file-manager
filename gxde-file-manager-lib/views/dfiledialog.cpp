    ...
    , d_ptr(new DFileDialogPrivate())
{
    ...
    // DFileDialog does not go through WindowManager::loadWindowState(), so give
    // it an explicit logical size on Wayland before its first surface commit.
...
void DFileDialog::handleNewView(DFMBaseView *view)
{
    ...
    whitelist << DFMGlobal::NewFolder << DFMGlobal::NewDocument << DFMGlobal::DisplayAs << DFMGlobal::Refresh
